...
class GenerateePub extends Command
{
    ...
    public function handle()
    {
        ...
        if ($args["novel"] == 0) {
            Novel::where("status", 1)
                ->whereNull("epub_generated")
                ->whereHas("chapters")
                ->with([
                    "chapters" => function ($q) {
                        $q->where("blacklist", 0)
                ...
                ->chunk(5, function ($novels) {
                    foreach ($novels as $object) {
                        ...
                        // Generate ePub
                        $file_chapters = glob(
                            storage_path("app/Novel/" . $id . "/Text/*")
                        );
                        $file_content = storage_path(
                            "app/Novel/" . $id . "/content.opf"
                        );
                        $file_toc = storage_path(
                            "app/Novel/" . $id . "/toc.ncx"
                        );
                ...
                ->chunk(5, function ($novels) {
                    foreach ($novels as $object) {
                        ...
                        if (
                            file_exists(
                                storage_path(
                                    storage_path("app/Novel/" . $object->id)
                                )
                            )
                        ) {
